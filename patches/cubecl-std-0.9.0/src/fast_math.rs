...
fn find_params_u32(divisor: u32) -> (u32, u32) {
    // Guard: zero-size tensor dimensions reach here when Tensor::cat includes an empty tensor.
    // The kernel launches 0 threads for that slice, so the values are never used.
    if divisor == 0 {
        return (0, 1);
    }
